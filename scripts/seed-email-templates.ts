...
import { createDatabaseTemplateRepository } from '../src/lib/server/database/repositories/email-template-repository-factory';
import type { CreateTemplateData } from '../src/lib/server/database/types/email-templates';
...
async function seedEmailTemplates() {
	...
	const repository = createDatabaseTemplateRepository({
		enableCache: false
	});
	...
	for (const [type, template] of Object.entries(builtInTemplates)) {
		...
		try {
			const existing = await repository.getTemplateByType(templateType);
			...
			await repository.createTemplate(templateData);
